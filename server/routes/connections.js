...
const searchFields = 'Id,LastNameAtBirth,LastNameCurrent,FirstName,Name,BirthDate,DeathDate,BirthLocation,DeathLocation,Derived.LongName,Derived.LongNamePrivate,Manager,IsLiving,Privacy'
...
connRouter.get("/getZalewskiPeople/:page(\\d+)?", async (req, res) => {
    let pageLength = req.query.pagesize || 50;
    ...
    await fetch(`https://api.wikitree.com/api.php?action=searchPerson&start=${offset}&LastName=Zalewski&fields=${searchFields}&skipVariants=0&sort=birth&lastNameMatch=birth&limit=${pageLength}`)
    .then(response => response.json())
    .then(json => {
        res.send(json);
    })
